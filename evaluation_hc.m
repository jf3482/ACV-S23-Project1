...
%Linking results
gt_files = {gt.filename};
for i = 1:length(res)
    res(i).result = 0;
            
    txt = res(i).image_id;
    ind_gt = find(strcmp(gt_files,txt));
%     disp(res(i))
    y = (res(i).bbox_3 + res(i).bbox_1) /2;
    x = (res(i).bbox_4 + res(i).bbox_2) /2;
%     disp(x)
%     disp(y)
    for j = 1:length(ind_gt)
        gt_box = str2num(gt(ind_gt(j)).bbox);
        if x>=gt_box(1) && x<=gt_box(3) && y>=gt_box(2) && y<=gt_box(4)
            res(i).result = 1;
            res(i).linkgt = ind_gt(j);
            break;
        end
    end
end
